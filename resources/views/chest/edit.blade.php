...
<!-- <input type="hidden" name="_token" value="{{{ csrf_token() }}}" /> -->
  <div class="form-group{{ $errors->has('chestName') ? ' has-error' : '' }}">
    ...
    <input type="text" class="form-control" id="chestName" name="chestName" placeholder="Your new chest name"  value="{{$cufar->name}}">
  ...
  <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
    ...
    <textarea rows="3" class="form-control" id="description" name="description" placeholder="Brief description .." >{{$cufar->description}}</textarea>
